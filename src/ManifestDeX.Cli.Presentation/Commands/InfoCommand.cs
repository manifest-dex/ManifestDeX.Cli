...
public sealed class InfoCommand : AsyncCommand<InfoCommand.Settings>
{
    private readonly GetGameInfoUseCase _useCase;

    public InfoCommand(GetGameInfoUseCase useCase)
    {
        _useCase = useCase;
    }
    ...
    protected override async Task<int> ExecuteAsync(CommandContext context, Settings settings, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var info = await _useCase.ExecuteAsync(appId, cancellationToken);
            ConsoleOutput.PrintObject(info, settings.ResolveOutputMode());
